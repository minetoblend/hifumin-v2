...
import com.minetoblend.osugachabot.discord.application.burn.BurnDialog.BurnStatus.Cancelled
import com.minetoblend.osugachabot.discord.application.burn.BurnDialog.BurnStatus.Confirmed
import com.minetoblend.osugachabot.discord.application.burn.BurnDialog.BurnStatus.Pending
...
class BurnDialog(
    private val interaction: ActionInteraction,
    private val replica: CardReplica,
    ...
    fun MessageBuilder.render() {
        embed {
            ...
            when (status) {
                Cancelled -> footer { text = "Burn has been cancelled" }
                Confirmed -> footer { text = "Card has been burned" }
                else -> {}
            }
        }
        ...
        if (status == Pending) {
            actionRow {
                interactionButton(ButtonStyle.Primary, "card:burn:confirm") { label = "Confirm" }
                interactionButton(ButtonStyle.Danger, "card:burn:cancel") { label = "Cancel" }
            }
        }
    }
    ...
    suspend fun run() {
        val response = coroutineScope {
            ...
            ack.respond {
                addFile("card.png", ChannelProvider { ByteReadChannel(image) })
                render()
            }
        }

        scope.launch(Context.root().asContextElement()) {
            val job = launch { receiveButtonEvents(response) }
            delay(2.minutes)
            job.cancel()
            response.edit { components = mutableListOf() }
        }
    }
    ...
    private fun receiveButtonEvents(response: PublicMessageInteractionResponse) {
        interaction.kord.on<ButtonInteractionCreateEvent> {
            if (interaction.message.id != response.message.id) return@on
            if (interaction.user.id != this@BurnDialog.interaction.user.id) {
    ...
    enum class BurnStatus { Pending, Confirmed, Cancelled }
}
